...
"""
RetinaNet 객체 탐지기 구현 (PyTorch 기반)
torchvision 패키지를 사용하여 RetinaNet 모델로 객체 탐지 수행
"""
...
class RetinaNetDetector:
    ...
    def __init__(self, model_path=None, conf_thres=0.5, iou_thres=0.45, device='', classes=None):
        ...
        # RetinaNet 모델 로드
        try:
            logger.info(f"RetinaNet 모델 로드 중...")
            ...
            if model_path and os.path.exists(model_path):
                ...
                logger.info("사전 학습된 RetinaNet 모델 로드 완료")
            ...
            logger.info(f"RetinaNet 모델 로드 완료 (device: {self.device})")
            
        except Exception as e:
            logger.error(f"RetinaNet 모델 로드 실패: {e}")
            raise
