...
"""
YOLOv5 객체 탐지기 구현 (Ultralytics 기반)
ultralytics 패키지를 사용하여 YOLOv5 모델로 객체 탐지 수행
"""
...
class YOLOv5Detector:
    ...
    def __init__(self, model_path=None, conf_thres=0.5, iou_thres=0.45, device='', classes=None):
        ...
        try:
            ...
            if not model_path:
                model_path = '../models/object_detector_models/yolov5x.pt'  # 기본 가중치
                
            logger.info(f"YOLOv5 모델 로드 중: {model_path}")
            self.model = YOLO(model_path)
            
            # 디바이스 설정
            self.model.to(self.device)
            
            logger.info(f"YOLOv5 모델 로드 완료 (device: {self.device})")
