...
"""
Mask R-CNN 객체 탐지기 구현 (PyTorch 기반)
torchvision 패키지를 사용하여 Mask R-CNN 모델로 객체 탐지 수행
"""
...
class MaskRCNNDetector:
    """Mask R-CNN 객체 탐지기 클래스"""
    
    def __init__(self, model_path=None, conf_thres=0.5, iou_thres=0.45, device='', classes=None):
        self.conf_thres = conf_thres
        self.iou_thres = iou_thres
        self.classes = classes if classes is not None else [1]  # 기본 Featured Image 기본값: 사람 클래스만 탐지 (COCO에서 ID 1)
        ...
        try:
            logger.info("Mask R-CNN 모델 로드 중...")
            if model_path and os.path.exists(model_path):
                ...
                logger.info("사전 학습된 Mask R-CNN 모델 로드 완료")
            
            self.model.eval()
            self.model = self.model.to(self.device)
            logger.info(f"Mask R-CNN 모델 로드 완료 (device: {self.device})")
        
        except Exception as e:
            logger.error(f"Mask R-CNN 모델 로드 실패: {e}")
            raise
